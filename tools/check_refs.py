...
def main():
    ...
    # 1. 上标链接格式（硬错误）
    # 纯文本 sup（无链接）
    plain = re.findall(r'<sup>\[([\d-]+)\]</sup>(?!<a)', zh_body)
    if plain:
        errors.append(f"[中文] 纯文本上标（缺链接）: {sorted(set(plain))[:5]}")
    # 绝对路径链接（GitHub 会坏）
    abslink = re.findall(r'<sup><a href="/12-[^"]*#([\d-]+)">', zh_body)
    if abslink:
        errors.append(f"[中文] 绝对路径链接（GitHub 会坏）: {sorted(set(abslink))[:5]}")
    # 链接 href 形式检查
    bad_href = re.findall(r'<sup><a href="([^"]*)"', zh_body)
    ok_zh = re.findall(r'<sup><a href="12-附录A-案例索引与资料出处\.md#[\d-]+">', zh_body)
    if len(ok_zh) != len(bad_href):
    ...
    missing = sorted(r for r in zh_refs if r not in appendix_refs)
    if missing:
        errors.append(f"[中文] 悬空引用（附录A无此编号）: {missing[:10]}")

    # 3. 附录有条目正文未引用（警告，需人工确认是否漏标/延伸阅读）
    unused = sorted(appendix_refs - zh_refs)
    if unused:
        warnings.append(f"附录 A 有条目正文未标注 ({len(unused)} 条): {unused[:15]}{'...' if len(unused) > 15 else ''}")

    # 输出
    print("=" * 60)
    print("引用一致性自检（AI 客户经营）")
    print(f"附录 A 条目: {len(appendix_refs)} | 正文引用: {len(zh_refs)}")
    print("=" * 60)
    if errors:
        for e in errors:
            print(f"❌ {e}")
